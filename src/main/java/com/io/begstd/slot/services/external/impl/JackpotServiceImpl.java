...
import com.io.begstd.slot.grpc.jackpotservice.*;
...
public class JackpotServiceImpl implements JackpotService {
    ...
    private JackpotServiceGrpc.JackpotServiceBlockingStub stub = null;
    private String serviceId = null;
    private static final ReentrantLock lock = new ReentrantLock();

    public JackpotServiceGrpc.JackpotServiceBlockingStub intial() {
        if (stub == null) {
            try {
                lock.lock();
                if (stub == null) {
                    ManagedChannel channel = ManagedChannelBuilder
                            ...
                            .usePlaintext().build();
                    stub = JackpotServiceGrpc.newBlockingStub(channel);
                }
            } finally {
                lock.unlock();
            }
    ...
    public boolean plusJackpot(String commandID, String jackpotID, double money) {
        ResponseStatusJP status = null;
        try {
            status = intial().plus(
                PlusRequest.newBuilder().setCommandId(commandID).setJackpotId(jackpotID).setAmount(money).build());
    ...
    public Money awardJackpot(String commandID, String jackpotID, double money) {
        ...
        AwardResponse status = null;
        try {
            AwardRequest.Builder awardBuilder = AwardRequest.newBuilder()
...
//            log.info("Award Jackpot status response {}", status.getCode());
    ...
    public boolean initJackpot(String prefixService, String gameId, String commandID, Map<String, Money> JACKPOTS) {
        ...
        ResponseStatusJP status = null;
        try {
            InitRequest.Builder initBuilder = InitRequest.newBuilder();
            ...
            for (Map.Entry<String, Money> entry : JACKPOTS.entrySet()) {
                JackpotInfo.Builder jackpotIInfo = JackpotInfo.newBuilder();
...
//            log.error("ERROR -- initJackpot commandID {} - jackpotID {} -- Money {} - error {}", commandID, gameId, e);
    ...
    public boolean plusMultipleJackpots(String commandID, Map<String, Money> JACKPOTS) {
        ...
        ResponseStatusJP status = null;
        try {
            MultiplePlusRequest.Builder initBuilder = MultiplePlusRequest.newBuilder();
            initBuilder.setCommandId(commandID);
...
//            log.error("ERROR -- initJackpot commandID {} - jackpotID {} --  error {}", commandID, JACKPOTS.toString(), e);
    ...
    public Map<String, Money> plusMultipleJackpotsWithAmount(String commandID, Map<String, Money> JACKPOTS) {
        ...
        JackpotMultiResponseStatus status = null;
        try {
            MultiplePlusRequest.Builder initBuilder = MultiplePlusRequest.newBuilder();
            initBuilder.setCommandId(commandID);
...
//            log.error("ERROR -- initJackpot commandID {} - jackpotID {} --  error {}", commandID, JACKPOTS.toString(), e);
    ...
    public Money getJackpot(String commandId, String jackpotID) {
        ...
        GetJackpotResponse status = null;
        try {
            status = intial().getJackpot(GetJackpotRequest.newBuilder().setJackpotId(jackpotID).build());
