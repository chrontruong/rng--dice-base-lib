...
public abstract class BaseGambleGameService extends BaseSpinService {

    @Autowired
    private UserService userService;

    public BasePlaySession spin(String commandId, UserInfo userInfo, int openCell, double totalBet) {
        return spin(commandId, userInfo, openCell, totalBet, false);
    }
    
    public BasePlaySession spinTrial(String commandId, UserInfo userInfo, int openCell, double totalBet) {
        return spin(commandId, userInfo, openCell, totalBet, true);
    }
    
    private BasePlaySession spin(String commandId, UserInfo userInfo, int openCell, double totalBet, boolean isTrialMode) {
        ...
        logBuilder
            ...
            .stateName("BaseGambleGameService.spin");

//        checkAuthorize(slotMachineConfigNormal.serviceId(), userId, commandId);
        BasePlaySession basePlaySession = userService.getGamblePlaySession(slotMachineConfigNormal.serviceId(), userInfo);
        if(basePlaySession != null && !basePlaySession.isTrialMode() && isTrialMode) {
            logBuilder.serviceId(basePlaySession.serviceId())
                ...
                .message("ERROR CALL SERVICE GAMBLE TRIAL MODE FOR A REAL PLAYSESSION FOR USER");
        ...
        if(basePlaySession != null && basePlaySession.isTrialMode() && !isTrialMode) {
            logBuilder.serviceId(basePlaySession.serviceId())
                .psId(basePlaySession.uuid()).stepName("isTrialMode")
                .message("ERROR CALL SERVICE GAMBLE MODE FOR A TRIAL PLAYSESSION FOR USER");
            LogsUtils.writeLogError(logBuilder.build());
            throw new SlotGameException(SlotGameError.UNEXPECTED, userInfo.userId(), basePlaySession.userType(), commandId);
        }
        
//        PlaySession playSession = userService.getPlaySession(slotMachineConfigNormal.serviceId(), userId);
        if ((basePlaySession != null) && (basePlaySession.userId().equals(userInfo.userId()))) {
            BasePlaySession basePlaySessionAfterSpin = execute(basePlaySession, commandId, userInfo, openCell, totalBet, isTrialMode, configManager.getMainSlotConfigMap());
    ...
    protected abstract BasePlaySession execute(BasePlaySession basePlaySession, String commandId, UserInfo userInfo,
                                               int openCell, double totalBet, boolean isTrialMode,
