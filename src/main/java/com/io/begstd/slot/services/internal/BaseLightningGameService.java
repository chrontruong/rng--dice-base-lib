...
public abstract class BaseLightningGameService extends BaseSpinService {
    ...
    public BasePlaySession spin(String commandId, UserInfo userInfo, boolean isTrialMode) {
//        checkServiceId(slotMachineConfigForFree.serviceId(), serviceId, userId, commandId);
        ...
        logBuilder
            ...
            .stateName("BaseLightningGameService.spin");
        
//        checkAuthorize(slotMachineConfigForNormal.serviceId(), userId, commandId);
        BasePlaySession basePlaySession = userService.getPlaySession(slotMachineConfigForNormal.serviceId(), userInfo);
        if(basePlaySession != null && !basePlaySession.isTrialMode() && isTrialMode) {
            logBuilder.serviceId(basePlaySession.serviceId())
                ...
                .message("ERROR CALL SERVICE LINGHTNING TRIAL MODE FOR A REAL PLAYSESSION FOR USER");
        ...
        if(basePlaySession != null && basePlaySession.isTrialMode() && !isTrialMode) {
            logBuilder.serviceId(basePlaySession.serviceId())
                .psId(basePlaySession.uuid()).stepName("isTrialMode")
                .message("ERROR CALL SERVICE LINGHTNING GAME TRIAL MODE FOR A TRIAL PLAYSESSION FOR USER");
            LogsUtils.writeLogError(logBuilder.build());
            throw new SlotGameException(SlotGameError.UNEXPECTED, userInfo.userId(), basePlaySession.userType(), commandId);
        }
