...
/**
 * 
 * Don't use for new game, new game with use BonusGameOrder rule
 *
 */
@Component("bonusGameOrder")
@Slf4j
public class MiniGameOrder implements WonRuleExtension {
    public static final String BONUSGAME_ORDER = "bonusGameOrder";
    @Override
    ...
    public BasePlaySession calculatorPayout(CalculatePayoutArgs calculatePayoutArgs, BasePlaySession basePlaySession) {
        ...
        if (freeSpinAward == 1) {
            //win bonus
            if (symBonus.paytable() != null) {
                winBonus = symBonus.paytable().get(resultCheck.size() - 1);
            }
        }
