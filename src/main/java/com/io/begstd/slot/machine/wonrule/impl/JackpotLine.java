...
@Slf4j
@Component("jackpotLine")
public class JackpotLine implements WonRuleExtension {

    public static final String JACKPOTLINE = "jackpotLine";
    @Autowired
...
//            builder.payLineJackpot(payLineList);
        ...
        logBuilder.cmdId(basePlaySession.commandId())
            ...
            .message("Jackpot amount = " + winJackpot.getValue1() + " - " + winJackpot.getValue0()
                        +" - isTrialMode: "+ basePlaySession.isTrialMode())
            .timeExe(0);
    ...
    private Pair<String, Money> calculateWonByJackPot(CalculatePayoutArgs calculatePayoutArgs,
        ...
        for (Symbol item : rowMatchBetLine) {
            if ((item != null) && (SymbolType.WILD.equals(item.type()))) {
                countWild++;
                wild = item;
            } else {
                break;
        ...
        if (countWild >= calculatePayoutArgs.config().jackpotLineSize()) {
            ...
            if(basePlaySession.isTrialMode()) {
                amountItem = jackpotTrialModeService.awardJackpot(basePlaySession.userId(), "commandIdJackpotLine for trial mode",  jackpotId,
                ...
                amountItem = jackpotService.awardJackpot("commandIdJackpotLine",  jackpotId,
                        calculatePayoutArgs.denomLevel().amount().multiply(initAmountJackpot).value().doubleValue());
