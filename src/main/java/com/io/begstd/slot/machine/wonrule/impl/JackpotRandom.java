...
@Component("jackpotRandom")
public class JackpotRandom implements WonRuleExtension {
    public static final String JACKPOTRANDOM = "jackpotRandom";
    
    @Override
    public String getName() {
        return JACKPOTRANDOM;
    }
    ...
    private BasePlaySession updatePlaySession(BasePlaySession basePlaySession, Pair<String, Money> winJackpot, List<String> payLineList) {
        ...
        if (winJackpot.getValue1().isGreaterThan(Money.of(1))) {
            ...
            builder.addJackpotInfo(winJackpot.getValue0(), winJackpot.getValue1(), payLineList.toString());
        ...
        for (int row = 0; row < calculatePayoutArgs.matrixScreen().rowSize(); row++) {
            for (int col = 0; col < calculatePayoutArgs.matrixScreen().reelSize(); col++) {
                if (SymbolType.WILD == viewMatrix[row][col].type()) {
                   countWild +=1;
                   payLineList.add(viewMatrix[row][col].code());
                }
            }
        }
        String jackpotId = "";
...
//                log.debug("Win Jackpot Random trial mode playsessionId: "+playSession.uuid()+" -- userId: "+playSession.userId()+" -- jackpotId:"+jackpotId+" amount: "+amountResult);
            } else {
