...
/**
 * Multiplier with Wild symbol
 * */
@Slf4j
@Component("allWayToWinMultipler")
public class AllWayToWinMultiplier implements WonRuleExtension {
    public static final String ALLWAYTOWIN_MULTIPLIER = "allWayToWinMultipler";
    @Autowired
    private Environment environment;
    
    @Override
    public String getName() {
        return ALLWAYTOWIN_MULTIPLIER;
    }
...
//                                + (pairItem.getValue1()) + ")" +" multiplierValue:" + calculatePayoutArgs.multiplier());
                        dataBuffer.append("multiplierValue:").append(calculatePayoutArgs.multiplier());
                        LogMessage.LogMessageBuilder logBuilder = LogMessage.builder();
                        logBuilder.cmdId(basePlaySession.commandId())
                            .actorId(basePlaySession.userId())
                            .serviceId(basePlaySession.serviceId())
                        ...
                        if(Arrays.asList(environment.getActiveProfiles()).contains("rtpdebug")) {
                            MatrixUtil.logMatrixRTP(calculatePayoutArgs.matrixScreen().horizontalMatrixDataCell(), "allWayToWinMultipler", Arrays.asList(3,3,3,3,3));
                      
                            log.error("Amount:" + amountLine + "-- Win (" + pairItem.getValue0() + "--:"
                                    + (pairItem.getValue1()) + ")" +" --multiplierValue:"+calculatePayoutArgs.multiplier()+" -- wonRate:"+wonRate+" -- winReels:"+(pairItem.getValue2()+1));
                        }
                    }
                }
            }
            ...
            if (countSymbol.type() == SymbolType.SYMBOL) {
                ...
                for (int col = 0; col < calculatePayoutArgs.matrixScreen().reelSize(); col++) {
                    ...
                    if (wildSymbol != null) {
                        ...
                        if (frequenceWild > 0) {
                            //calculate for multiplier WILD
                            frequenceWild = (int)calculatePayoutArgs.multiplier()*frequenceWild;;
                        }
                    }
