...
public abstract class BaseSpinService {
    ...
    protected void checkAuthorize(String serviceId, String userId, String commandId) {
        if (!userService.isJoinedGame(serviceId, UserInfo.builder().userId(userId).build())) {
            throw new DiceGameException(DiceGameError.NO_JOIN_GAME, userId, null, commandId);
        }
    }

    protected BasePlaySession finishPlaySession(BasePlaySession basePlaySession, IDiceMachineConfig slotMachineConfigNormal, UserInfo userInfo) {
        String addResult = "";
